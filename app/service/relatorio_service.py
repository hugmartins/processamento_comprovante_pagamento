...
class RelatorioService:
    ...
    def __init__(self):
        self.diretorio_resources = '../jasper_report/datasource/'
        self.diretorio_reports = '../jasper_report/report/'
        self.diretorio_comprovante_por_filial = '../output/comprovantes/'
        self.diretorio_relatorio_resultado_processamento = '../output/resultado_processamento_comprovantes/'
        self.diretorio_relatorio_inconsistencias = '../output/relatorio_inconsistencias/'
        self.diretorio_imagens = '../jasper_report/img/'

    def gerar_relatorio_comprovante(self, map_funcionarios_comprovante: dict):
        for codigo_filial in map_funcionarios_comprovante:
            ...
            for funcionario_filial in map_funcionarios_comprovante[codigo_filial]:
                ...
                detalhe_comprovante_report = self.__converter_funcionario_para_detalhe_report_comprovante(
                    funcionario_filial)
            ...
            self.__montar_parametros_para_gerar_relatorio_comprovante_pagamento(
                codigo_filial,
                data_geracao_arquivo_pagamento,
                nome_arquivo_datasource)

    def gerar_relatorio_resultado_processamento(self, total_funcionarios_por_filial: dict,
        ...
        relatorio_resultado_processamento = \
            self.__criar_report_resultado_processamento(total_funcionarios_por_filial,
        ...
        ArquivoService().criar_arquivo_datasource_resultado_processamento(nome_datasource_resultado_processamento,
                                                                          relatorio_resultado_processamento)
        self.__montar_parametros_para_gerar_relatorio_resultado_processamento(nome_datasource_resultado_processamento)

    def __criar_report_resultado_processamento(self, total_funcionarios_por_filial: dict,
                                               dados_funcionarios_com_comprovante_por_filial: dict,
                                               dados_funcionarios_sem_comprovante_por_filial: dict) -> \
            ReportResultadoProcessamento:
        ...
        for codigo_filial in total_funcionarios_por_filial:
            ...
            if len(lista_funcionarios_sem_comprovante) > 0:
                lista_funcionarios_ordenada = sorted(lista_funcionarios_sem_comprovante,
                                                     key=attrgetter('nome_completo'))
                for funcionario in lista_funcionarios_ordenada:
                    cpf = formatar_cpf_funcionario(funcionario.cpf)
                    resultado_processamento_funcionario = self.__gerar_detalhe_report_resultado_processamento(
                ...
                resultado_processamento_filial = self.__gerar_detalhe_report_resultado_processamento(
                    nome_filial, quantidade_funcionarios_filial, total_funcionario_com_comprovante,
    ...
    @staticmethod
    def __gerar_detalhe_report_resultado_processamento(nome_filial: str,
    ...
    def __converter_funcionario_para_detalhe_report_comprovante(self,
                                                                funcionario: Funcionario) -> DetalheReportComprovante:
        cpf = formatar_cpf_funcionario(funcionario.cpf)
        agencia_pagamento = self.__gerar_agencia_ou_conta_com_digito_verificador(
        ...
        conta_pagamento = self.__gerar_agencia_ou_conta_com_digito_verificador(
    ...
    @staticmethod
    def __gerar_agencia_ou_conta_com_digito_verificador(agencia_ou_conta: str, digito_verificador: int) -> str:
        if digito_verificador is None:
            digito_verificador = 0

        return ''.join((agencia_ou_conta, '-', str(digito_verificador)))

    def gerar_relatorio_inconsistencias(self, map_funcionarios_por_filial: dict):
        for codigo_filial in map_funcionarios_por_filial:
            ...
            lista_report_inconsistencia = self.__converter_funcionario_para_detalhe_report_inconsistencia(
            ...
            ArquivoService().criar_arquivo_datasource_inconsistencia_pagamento(nome_arquivo_datasource,
                                                                               comprovante_report)
            self.__montar_parametros_para_gerar_relatorio_inconsistencias(codigo_filial, nome_arquivo_datasource)

    @staticmethod
    def __converter_funcionario_para_detalhe_report_inconsistencia(codigo_filial: str,
                                                                   map_funcionarios_por_filial: dict) -> \
    ...
    def __montar_parametros_para_gerar_relatorio_comprovante_pagamento(self, codigo_filial: str,
                                                                       data_geracao_arquivo: str,
                                                                       nome_datasource: str):
        try:
            input_file = os.path.join(self.diretorio_reports, 'comprovante_pagamento_bradesco.jrxml')
            output_file = os.path.join(self.diretorio_comprovante_por_filial,
            ...
            parametros = {
                "logo_bradesco": os.path.abspath(os.path.join(self.diretorio_imagens, 'logo_bradesco.png')),
                "data_emissao_relatorio": data_atual_formatada('%d/%m/%Y')
            }
            self.__gerar_relatorio_jaspersoft(input_file, output_file, nome_datasource, nome_colunas_csv, parametros)
    ...
    def __montar_parametros_para_gerar_relatorio_resultado_processamento(self, nome_datasource: str):
        try:
            input_file = os.path.join(self.diretorio_reports, 'relatorio_processamento_comprovante.jrxml')
            output_file = os.path.join(self.diretorio_relatorio_resultado_processamento,
            ...
            parametros = {
                "logo_cbm": os.path.abspath(os.path.join(self.diretorio_imagens, 'logo_cbm.png')),
    ...
    def __montar_parametros_para_gerar_relatorio_inconsistencias(self, codigo_filial: str, nome_datasource: str):
        try:
            input_file = os.path.join(self.diretorio_reports, 'relatorio_inconsistencias.jrxml')
            nome_output_file = f'{codigo_filial} - {data_atual_formatada("%d_%m_%Y")} - INCONSISTENCIAS PAGAMENTO'
            output_file = os.path.join(self.diretorio_relatorio_inconsistencias, nome_output_file)
            ...
            logging.info(
                f'Relatorio de inconsistencias da filial {codigo_filial} gerado com sucesso. {output_file}.pdf')
        except Exception as error:
            finalizar_programa_error(f'Erro ao tentar gerar PDF a partir do datasource {nome_datasource}. {error}')

    def __gerar_relatorio_jaspersoft(self, input_file: str, output_file: str, nome_datasource: str,
                                     nome_colunas_csv: list,
                                     parametros: dict):
        conn = {
            'driver': 'csv',
            'data_file': os.path.abspath(os.path.join(self.diretorio_resources, f'{nome_datasource}.csv')),
            'csv_charset': 'utf-8',
